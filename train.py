...
def main():
    ...
    # setproctitle('SCS-{}'.format(args['probType']))
    ...
    config = {
        ...
        'simpleVar': args['simpleVar'],
        'simpleIneq': args['simpleIneq'],
        'simpleEq': args['simpleEq'],
        'simpleEx': args['simpleEx'],
        'epochs': args['epochs'],
    ...
    if log_wandb:
        with wandb.init(project='SCS_unroll', config=config, dir='/wandb/', name=args['probType']):
            print(wandb.run.dir)
...
def load_data(filepath, index):
    ...
    #     'X': X, 'Y': Y, 'S': S, 'iter': ITER, 'obj': OBJ_VAL, 'W': np.array(W)}
    data = {'P': P, 'c': c, 'A': A, 'b': b, 'G': G, 'h': h, 'l': l, 'u': u, 
...
def train_net(filepath, args):
    ...
    for i in range(nepochs):
        ...
        if log_wandb:
            with torch.no_grad():
                wandb.log({'train_loss': train_loss.item()/num_batch, 'epoch': i})
                wandb.log({'train_obj': data.obj_fn(idx, X).mean().item(), 'epoch': i})
                wandb.log({'train_eq_dist': data.eq_dist(idx, X).mean().item(), 'epoch': i})
                wandb.log({'train_ineq_dist': data.ineq_dist(idx, X).mean().item(), 'epoch': i})
        ...
        with torch.no_grad():
            ...
            for ii in range(num_batch):
                ...
                if False:
                    for iii in range(X.shape[0]):
                        ...
                        # solver_ws = scs.SCS(scs_data, cone_dict, verbose=False, rho_x=1.0, scale=1.0, alpha=1.5, adaptive_scale=False, acceleration_lookback=0, normalize=False)
            ...
            if log_wandb:
                wandb.log({'valid_loss': valid_loss/num_batch, 'epoch': i})
                wandb.log({'valid_obj': data.obj_fn(idx, X).mean().item(), 'epoch': i})
                wandb.log({'valid_eq_dist': data.eq_dist(idx, X).mean().item(), 'epoch': i})
                wandb.log({'valid_ineq_dist': data.ineq_dist(idx, X).mean().item(), 'epoch': i})
                wandb.log({'ws_iter': ws_iter/num_valid, 'epoch': i})
