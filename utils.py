...
class SimpleProblem:
    ...
    def __init__(self, data, valid_num=100, test_num=100, calc_X=False, device="cpu"):
        self.P = torch.tensor(data['P'], dtype=torch.float64) #/ 20  #/ 4000 # with shape n_samples x n_var x n_var
        self.c = torch.tensor(data['c'], dtype=torch.float64) #/ 20  # with shape n_samples x n_var
        self.A = torch.tensor(data['A'], dtype=torch.float64) #/ 100   # with shape n_samples x n_eq x n_var
        self.b = torch.tensor(data['b'], dtype=torch.float64) #/ 100  # with shape n_samples x n_eq
        self.G = torch.tensor(data['G'], dtype=torch.float64) #/ 1000
        self.h = torch.tensor(data['h'], dtype=torch.float64) #/ 1000
        ...
        # self.data = data
        # replace P to (P + P.T) / 2, the scs solver requires a psd quadratic coefficient matrix
        ...
        if self.l is not None:
            ...
            # release lb_idx, A_lb, b_lb
            # del lb_idx, A_lb, b_lb
    ...
    #     primal = self.my_A.bmm(X) - self.my_b.unsqueeze(-1)
    #     primal = torch.cat((primal[:, :self.n_eq], torch.clamp(primal[:, self.n_eq:], min=0)), dim=1)  # Concatenate along the second dimension
    #     # only keep the positive values
    #     # primal = torch.clamp(primal, min=0)
    #     dual = self.P.bmm(X) + self.my_A.transpose(1, 2).bmm(Y) + self.c

    #     return torch.linalg.norm(primal, ord=2, dim=(1,2)) + torch.linalg.norm(dual, ord=2, dim=(1,2))
    ...
    def DR_gd_torch(self, max_iter=1, tol=1e-4, w=None, u_tilde=None):
        ...
        for i in range(max_iter):      
            ...
            v = u + w - 2 * u_tilde
        # return u[:, :self.n_var], u[:, self.n_var:], v[:, self.n_var:]
        return u, v
    ...
    def scs_solve(self, batch, tol=1e-4, verbose=True, accelerate=True):
        ...
        # return np.array(X), np.array(Y), np.array(S), np.array(iters), np.array(sol_time), np.array(obj_val)
