...
def main():
    ...
    parser.add_argument('--probType', type=str, default='simple_rhs_100', #default='qplib_8616',
                        help='problem type')
...
def test_net(filepath, args):
    ...
    for idx in test_indices:
        ...
        while gap > eps_abs_target + eps_rel_target * scale and eps_abs > 1e-10 and eps_rel > 1e-8:
            problem = osqp.OSQP()
            # problem.setup(P=csc_matrix(P), q=c, A=csc_matrix(A), l=l, u=u, check_termination=1, verbose=False, max_iter=100000, eps_abs=eps_abs, eps_rel=eps_rel, eps_prim_inf=1e-7, eps_dual_inf=1e-7)
