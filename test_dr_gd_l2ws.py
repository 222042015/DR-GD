...
def DR_gd_torch(M, q, n, m, max_iter=10000, tol=1e-4, w=None, u_tilde=None):
    ...
    for i in range(max_iter):        
        ...
        for _ in range(1):
            ...
            ##################################
            # res = u_tilde + M.bmm(u_tilde) - b
            # grad = res + M.transpose(1,2).bmm(res)
            # AAT_res = grad + M.bmm(grad)
            # eta = torch.sum((res * AAT_res).squeeze(-1), dim=1) / torch.sum((AAT_res * AAT_res).squeeze(-1), dim=1)
            # u_tilde = u_tilde - eta.unsqueeze(-1).unsqueeze(-1) * grad
        ...
        if i % 1 == 0:
            ...
            # print(error)
            if error < tol:
                # print("DR Splitting-GD stopped at iteration: ", (i+1))
                break

        w_prev = w.clone()
            
    # if i == max_iter - 1:
    #     print("DR-GD Splitting stopped at maximum iteration: ", max_iter)
...
def DR_torch(M, q, n, m, max_iter=100000000, tol=1e-4, w=None,):
    if w is None:
        ...
        w = w.clone()
    # lu, pivots = torch.linalg.lu_factor(torch.eye(M.shape[1], device=DEVICE) + M)
    w_prev = w
    M_tmp = torch.linalg.inv(torch.eye(M.shape[0], device=DEVICE) + M)
    
    for i in range(max_iter):
        # u_tilde = torch.linalg.lu_solve(lu, pivots, (w - q).unsqueeze(-1)).squeeze(-1)
        # u_tilde = np.linalg.solve(np.eye(M.shape[0]) + M, w-q)
        u_tilde = M_tmp @ (w - q)
        u = 2 * u_tilde - w
        ...
        #     print(error.item())
        if error < tol:
            # print("DR Splitting stopped at iteration: ", (i+1))
            break
...
def DR_homo_torch(M, q, n, m, max_iter=100000, tol=1e-4, mu=None, eta=None):
    ...
    for i in range(max_iter):
        ...
        # Calculate error
        # error = torch.norm(mu - mu_prev, dim=1) + torch.norm(eta_prev - eta, dim=1)
...
def test_net(filepath, args):
    ...
    # test_indices = np.arange(num_examples)[num_train+num_valid:]
    ...
    # checkpoint_file = 'checkpoint_' + args['probType'] + '_' + str(args['train_unrolls']) + '.pth'
    if args['supervised']:
    ...
    for idx in test_indices:
        ...
        if args['timeMode'] == 'cpu':
            ...
            with torch.no_grad():
                ...
                # w = torch.zeros_like(w, device=DEVICE)
